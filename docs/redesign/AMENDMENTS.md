...
## Amendment 1 — keep Anton (owner, 2026-08-09)

The handoff's earlier display-face decision is reversed. Concretely:

- Keep the existing Anton import in `src/app/[locale]/layout.tsx` exactly as
  it is (`latin` + `latin-ext`, `--font-anton`). No alternate display-face
  import or variable may exist in the tree.
- `design-tokens.css` is already amended: `--font-display` resolves to Anton,
  ...
  the current tree — tighter multiline leading would clip diacritics on
  Anton), `--leading-display-tight` is 1.05 for guaranteed
  single lines.
...
- The wordmark and OG template (§11) are set in Anton 400. The wordmark still
  ships with text converted to outlines.
